...
Runtime commands: `ai-reviewer {doctor,serve,daemon,sync,review <ref>}`. There is no init step: dirs/DB are created on first use and `serve` shows a web setup screen (gated UI) until GitLab host+token and the `claude` CLI are in place. Run `doctor` after config changes — it verifies GitLab auth, the `claude` CLI, git, the DB, and FTS5. `<ref>` accepts a full MR URL, `group/sub/repo!123`, or `project-id:iid` (see `internal/gitlab/ref.go`).
...
- **Config**: `~/.ai-reviewer/config.yaml` (created automatically on first setup, `0600`; runtime edits go through `config.PatchFile`, which preserves comments and unrelated keys). Any field overridable by `AI_REVIEWER_<PATH>` env vars. The GitLab token is stored in `gitlab.token` in the config file by design (not env-only) — see the `gitlab-token-in-config` memory. `gitlab.token_env` (default `GITLAB_TOKEN`) is the fallback when `gitlab.token` is empty.
