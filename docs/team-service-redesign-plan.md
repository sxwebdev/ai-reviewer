...
> **Поправка от 2026-08-17 (по итогам первого прогона на живом GitLab).** Признаков
> четыре, а не три: первым идёт `🔁 changes requested by @…`. Списка из трёх было
> недостаточно — `ClassifyAuthorActions` вообще не смотрел на `Reviewers`, поэтому
> вердикт «Request changes» доходил до автора только случайно, если ревьюер заодно
> оставил резолвабельный тред. Без такого треда MR не попадал **ни в одну** секцию:
> `NeedsHumanReview` считал ревьюера отработавшим, а автору предъявить было нечего.
> Порядок строк теперь changes requested → threads → conflicts → pipeline.
>
> Правило парное: ровно одна секция забирает MR. Вердикт, на который автор уже
> ответил пушем, уходит из «Author actions» и возвращает ревьюера в «Reviews
> needed»; вердикт без датируемой активности остаётся у автора. Если сломать одну
> половину, MR либо продублируется, либо исчезнет.
>
> Заодно: `UNAPPROVED` и `REVIEW_STARTED` не распознавались (`gitlab.ReviewState`
> их знал, `domain.ParseReviewState` — нет) и уходили в REST-фолбэк, который читает
> «оставил заметку после пуша» как «готово». На живом проекте это 7 состояний из 57.

> **Поправка от 2026-08-17 (вторая, по размеру дайджеста).** Двух секций больше нет —
> дайджест сгруппирован **по людям**, и один блок отвечает на «что должен этот
> человек»: сначала ревью, которые он не отдал, затем его собственные MR. Разметка
> выше устарела; актуальный вид описан в `docs/operations.md`.
>
> Причина — арифметика, а не вкус. Секции группировались независимо (по ревьюеру и
> по автору), поэтому один MR печатался под каждым своим ревьюером, а человек
> появлялся в двух местах. На реальном прогоне это **~100 строк и два сообщения на
> 25 MR**: `!1404` встречался 4 раза, `!1365` — 4, ещё пять MR — по 3. Плюс каждая
> запись занимала две строки (`by <автор> · waiting Nd`), а заголовки длиной до 104
> символов переносились.
>
> Что изменилось: группировка по людям; одна строка на MR; автор из строки ревью
> убран; заголовок обрезается до 55 символов по границе слова (ключ тикета впереди,
> поэтому выживает); подробно печатаются три самых старых ревью, остальные — строкой
> `+N more:` со ссылками, ничего не выкидывается; метка возраста 🔴 ≥7д / 🟡 ≥2д /
> ▫️ вместо `waiting`; на одно-проектном дайджесте имя проекта уходит в заголовок и
> не повторяется в каждой строке. Те же данные: **35 строк, одно сообщение**.
>
> Фильтров сознательно нет: старые MR, черновики и `38m` остаются в списке. Порядок
> людей — по объёму долга, больше сверху (алфавит прятал самую большую очередь).
...
> **Поправка от 2026-08-17 (третья, по итогам второго прогона).** «Второй сигнал —
> форс-выход» не работал, и причина в том, что обработчиков сигнала было два.
> `main` ставил `signal.NotifyContext`, лаунчер — свой `signal.Notify`; сигнал
> приходит в **оба** канала, а свой watcher форс-выхода mx включает только в той
> ветке, где выиграл его канал. Кто выигрывает — гонка: на трёх прогонах стенда
> один просидел весь дренаж, игнорируя все последующие сигналы (первая же
> регистрация снимает default disposition, и Ctrl-C больше никого не убивает).
> Владелец сигналов теперь один — `app.ShutdownContext`, у лаунчера
> `launcher.WithSignal(false)`; первый сигнал отменяет контекст, второй выходит с
> кодом 1.
>
> Дренаж при этом **не** распространяется на ревью: 4–10 минут против окна в 60
> секунд — закончить оно не может, а платить продолжает. `jobs.Service.withShutdown`
> отменяет ревью в момент начала остановки; строка попытки отбрасывается
> (`recordFailure` уже отличает нашу остановку от провала MR), и следующий скан
> ставит тот же SHA заново. Отсюда же — `prepareWorktree` больше не деградирует до
> diff-only, если зеркало упало из-за нашей же остановки: терминальный Ctrl-C
> убивает дочерний `git` через process group, и раньше это читалось как обычная
> инфраструктурная беда, после чего запускался полноценный LLM-проход на уже
> уходящем процессе.
