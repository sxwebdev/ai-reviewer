...
| `merge_requests_scanned_total{team}`               | counter: open MRs seen by a scan. **Flat at zero for a team whose `ai_review.enabled` is off** — such a team's repositories are never listed, so nothing is seen and nothing is counted. `ai_reviews_skipped_total{reason="disabled"}` stops for the same reason. Neither is a fault; `effective mode` at startup and `doctor` say which teams review |
